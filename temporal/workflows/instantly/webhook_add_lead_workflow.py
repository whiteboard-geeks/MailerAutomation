...
class WebhookAddLeadWorkflow:
    ...
    async def run(self, input: WebhookAddLeadPayload) -> None:
        ...
        if not campaign_name:
            _send_error_email_campaign_name_not_found(
                workflow_id=workflow.info().workflow_id,
                lead_id=input_validated.event.data.lead_id,
                task_text=input_validated.event.data.text,
            )
            raise ApplicationError(
                f"Could not extract campaign name from task: {input_validated.event.data.text}"
            )
    ...
    def _validate_input(input: WebhookAddLeadPayload) -> WebhookAddLeadPayloadValidated:
        ...
        if input_validated.event.action != "created":
            _send_error_email_action_not_created(
                workflow_id=workflow.info().workflow_id,
                lead_id=input_validated.event.data.lead_id,
                action=input_validated.event.action,
            )
            raise ApplicationError(
                f"Expected created action, got {input_validated.event.action}"
            )

        if input_validated.event.object_type != "task.lead":
            _send_error_email_object_type_not_task_lead(
                workflow_id=workflow.info().workflow_id,
                lead_id=input_validated.event.data.lead_id,
                object_type=input_validated.event.object_type,
            )
            raise ApplicationError(
                f"Expected task.lead object, got {input_validated.event.object_type}"
            )

        if not input_validated.event.data.text.lower().startswith("instantly:"):
            _send_error_email_task_does_not_start_with_instantly(
                workflow_id=workflow.info().workflow_id,
                lead_id=input_validated.event.data.lead_id,
                task_text=input_validated.event.data.text,
            )
            raise ApplicationError(
                f"Expected task to start with Instantly:, got {input_validated.event.data.text}"
            )

        return input_validated

    async def _add_lead_to_instantly_campaign(
        self, input: AddLeadToInstantlyCampaignArgs
    ) -> None:
        while True:
            try:
...
def _send_error_email_validation_error(
    ...
    detailed_error_message = f"""
        ...
        """
    send_email(
        subject="Add Lead Workflow: Payload Validation Error",
        body=detailed_error_message,
    )


def _send_error_email_campaign_name_not_found(
    workflow_id: str, lead_id: str, task_text: str
) -> None:
    detailed_error_message = f"""
        ...
        """
    send_email(
        subject="Add Lead Workflow: Campaign Name Not Found",
        body=detailed_error_message,
    )


def _send_error_email_action_not_created(
    workflow_id: str, lead_id: str, action: str
) -> None:
    detailed_error_message = f"""
        ...
        """
    send_email(
        subject="Add Lead Workflow: Action Not Created", body=detailed_error_message
    )


def _send_error_email_object_type_not_task_lead(
    workflow_id: str, lead_id: str, object_type: str
) -> None:
    detailed_error_message = f"""
        ...
        """
    send_email(
        subject="Add Lead Workflow: Object Type Not Task Lead",
        body=detailed_error_message,
    )


def _send_error_email_task_does_not_start_with_instantly(
    workflow_id: str, lead_id: str, task_text: str
) -> None:
    detailed_error_message = f"""
        ...
        """
    send_email(
        subject="Add Lead Workflow: Task Does Not Start With Instantly:",
        body=detailed_error_message,
    )
